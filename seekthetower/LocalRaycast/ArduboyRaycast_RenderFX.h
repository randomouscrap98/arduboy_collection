...
// #define RCSMALLLOOPS           // The raycaster makes use of loop unrolling,
// which adds about 1.5kb code. This removes that but performance severely drops

// Debug flags
// #define RCGENERALDEBUG       // Must be set for any of the othere to work
// #define RCLINEHEIGHTDEBUG      // Display information about lineheight (only
// draws a few lines) #define RCPRINTSPRITEDATA      // Display information
// about certain sprite-related properties

#ifdef RCGENERALDEBUG
#include <Tinyfont.h>
#endif

#define READTEXSTRIP(addr, out) FX::readDataObject<uint32_t>(addr, out)
// FX::readDataBytes(addr, (uint8_t *)&out, sizeof(uint32_t))
...
constexpr MipMapInfo MIPMAPS[8] PROGMEM = {
    ...
    {0, 32, 4},  {128, 16, 2}, {160, 8, 1}, {160, 8, 1},
    {168, 4, 1}, {168, 4, 1},  {168, 4, 1}, {168, 4, 1},
};
// ------------------------------------------------------------------------------
...
// A container for calculated sprite draw data. You know a calculation was not
// performed if stepX and stepY are 0
...
enum RcShadingType : uint8_t { None, Black, White };
...
// Raycast renderer container, tracks data used for raycasting + lets you render
// raycasting
template <uint8_t W, uint8_t H> class RcRender {
public:
  ...
  uflot lightintensity =
      1.0; // Impacts view distance + shading even when no shading applied
  uint24_t tilesheet;
  ...
  RcShadingType spriteShading =
      RcShadingType::None; // Sprite shading is kinda weird
  ...
  uflot _distCache[VIEWWIDTH /
                   2]; // Half distance resolution means sprites will clip 1
                       // pixel into walls sometimes but otherwise...
  ...
  // Draw a fast(?) raycast background, assumed to start at 0,0. Your background
  // should NOT have the usual width/height. Will underdraw in the Y direction
  // if VIEWHEIGHT not a multiple of 8
  ...
  // Calculate the appropriate shading for this wall slice given our rendering
  // config
  inline RcShadeInfo calculateShading(uflot distance, uint8_t x,
                                      RcShadingType shading) {
    if (shading == RcShadingType::None) {
      return RcShadeInfo{0xFF, RcShadingType::Black};
    } else {
      RcShadeInfo result{calcShading(distance, x, this->_darkness), shading};
  ...
  // Set the light intensity for raycasting. Performs several expensive
  // calculations, only set this when necessary
  ...
  void raycastWalls(RcPlayer *p, RcMap *map, Arduboy2Base *arduboy) {
    ...
    for (uint8_t x = 0; x < VIEWWIDTH; x++) {
      ...
      // The camera plane is a simple -90 degree rotation on the player
      // direction (as required for this algorithm).
      flot rayDirX = dX + dY * cameraX;
      flot rayDirY = dY - dX * cameraX;

      // length of ray from one x or y-side to next x or y-side. But we prefill
      // it with some initial data which has to be massaged later.
      ...
      // With this DDA stepping algorithm, have to be careful about making
      // too-large values with our tiny fixed point numbers. Make some
      // arbitrarily small cutoff point for even trying to deal with steps in
      // that direction. As long as the map size is never larger than 1 /
      // NEARZEROFIXED on any side, it will be fine (that means map has to be <
      // 100 on a side with this)
      ...
      do {
        ...
        if (sideDistX < sideDistY) {
          perpWallDist = sideDistX; // Remember that sideDist is actual distance
                                    // and not distance only in 1 direction
      ...
      // Only calc distance for every other point to save a lot of memory (100
      // bytes)
      if ((x & 1) == 0)
        distCache[x >> 1] = perpWallDist;

      // If the above loop was exited without finding a tile, there's nothing to
      // draw
      if (tile == RCEMPTY)
        continue;

      // Figure out NOW what the line height and mipmap level is is. Note: I've
      // tried many types for this invLineHeight, since it's used so much, but
      // float actually seems to work best...
      ...
      // NOTE: wallX technically can only be positive, but I'm using flot to
      // save a tiny amount from casting
      flot wallX = side ? fposX + (flot)perpWallDist * rayDirX
                        : fposY + (flot)perpWallDist * rayDirY;
      uint8_t texX = uint8_t((wallX - floorFixed(wallX)) *
                             mminfo.width); //.getFraction isn't what you think!
      if ((side == 0 && rayDirX > 0) || (side == 1 && rayDirY < 0))
        texX = mminfo.width - 1 - texX;

      UFixed<16, 16> step = mminfo.width * invLineHeight;
      uint16_t lineHeight = (invLineHeight <= MINLDISTANCE)
                                ? MAXLHEIGHT
                                : (uint16_t)(1 / invLineHeight);
      uint32_t texData;

      if ((side & x) && this->altWallShading != RcShadingType::None)
        texData =
            this->altWallShading == RcShadingType::Black ? 0x0 : 0xFFFFFFFF;
      else
        READTEXSTRIP(this->tilesheet + tile * 172 + mminfo.offset +
                         texX * mminfo.bytes,
                     texData);
      ...
      drawWallLine(x, lineHeight, step,
                   calculateShading(perpWallDist, x, this->shading), texData,
                   arduboy);
    }
  }

  // Draw a single raycast wall line. Will only draw specifically the wall line
  // and will clip out all the rest (so you can predraw a ceiling and floor
  // before calling raycast) void drawWallLine(uint8_t x, uflot distance,
  // RcShadeInfo shading, uint16_t texData, Arduboy2Base * arduboy)
  void drawWallLine(uint8_t x, uint16_t lineHeight, UFixed<16, 16> step,
                    RcShadeInfo shading, uint32_t texData,
    ...
    // Everyone prefers the high precision tiles (and for some reason, it's now
    // faster? so confusing...)
...
#define _WALLREADBYTE()                                                        \
  bofs = thisWallByte * WIDTH + x;                                             \
  texByte = sbuffer[bofs];
// Write previously read wall byte, go to next byte
#define _WALLWRITENEXT(mixin)                                                  \
  if (shading.type == RcShadingType::Black) {                                  \
    sbuffer[bofs] = (texByte & shade) mixin;                                   \
  } else {                                                                     \
    sbuffer[bofs] = (texByte | shade) mixin;                                   \
  }                                                                            \
  thisWallByte++;
// Work for setting bits of wall byte. Use an imperfect overflow accumulator to
// approximate stepping through texture.
#define _WALLBITUNROLL(bm, nbm)                                                \
  ...
  if (accum < accustep) {                                                      \
    ...
    uint8_t startByte =
        thisWallByte; // The byte within which we start, always inclusive
    ...
    if (yofs && startByte != endByte) {
      ...
      //"Don't repeat yourself": that ship has sailed. Anyway, only write the
      // last byte if we need to, otherwise we could legitimately write outside
      // the bounds of the screen.
      RCMASKBOTTOM(shading, shade, yofs);
      if (cornershading) {
        _WALLWRITENEXT(&~(fastlshift8(yofs)));
      } else {
        _WALLWRITENEXT();
      }
    }
    ...
    // Funny hack; code is written for loop unrolling first, so we have to kind
    // of "fit in" to the macro system
    if ((yStart & 7) == 0)
      thisWallByte--;
    ...
    do {
      ...
      // Every new byte, save the current (previous) byte and load the new byte
      // from the screen. This might be wasteful, as only the first and last
      // byte technically need to pull from the screen.
      if (bidx == 0) {
        bm = 1;
        _WALLWRITENEXT();
        _WALLREADBYTE();
    ...
    if (cornershading && yEnd != VIEWHEIGHT) {
      _WALLWRITENEXT(&~(fastlshift8(yEnd & 7)));
    } else {
      _WALLWRITENEXT();
    }
  ...
  // Precalculate some sprite drawing stuff, happens before any loop, not tied
  // to a sprite
  RcSpriteDrawPrecalc precalcSpriteDraw(RcPlayer *player) {
    ...
    result.invDet =
        1.0 / (player->dirY * player->dirY + player->dirX * player->dirX);
  ...
  RcSpriteDrawData calcSpriteDraw(RcSpriteDrawPrecalc *calc, RcPlayer *player,
    ...
    float transformYT =
        calc->invDet * (player->dirX * spriteX + player->dirY * spriteY);

    // Nice quick shortcut to get out for sprites behind us (and ones that are
    // too close / far).
    // NOTE: I added back in the back distance check. Why did I remove it???
    if (transformYT < MINSPRITEDISTANCE ||
        transformYT > _viewdistance + SPRITEVIEWEXENTSION)
      return result;
    //_sviewdistance)

    float invTransformYT = 1 / transformYT;
    float transformXT =
        calc->invDet * (player->dirY * spriteX - player->dirX * spriteY);

    // int16 because easy overflow! if x is much larger than y, then you're
    // effectively multiplying 50 by map width.
    //  NOTE: this is the CENTER of the sprite, not the edge (thankfully)
    int16_t spriteScreenX =
        int16_t(MIDSCREENX * (1 + transformXT * invTransformYT));

    // calculate the dimensions of the sprite on screen. All sprites are square.
    // Size mods go here using 'transformY' instead of the real distance
    // prevents fisheye
    float scale =
        (float)this->spritescaling[(sprite->state & RSSTATESIZE) >> 1];
    uint16_t spriteHeight = uint16_t(VIEWHEIGHT * invTransformYT * scale);
    uint16_t spriteWidth = spriteHeight;

    // calculate lowest and highest pixel to fill. Sprite screen/start X and
    // Sprite screen/start Y Because we have 1 fewer bit to store things, we
    // unfortunately need an int16
    int16_t ssX = -(spriteWidth >> 1) + spriteScreenX;
    // Offsets go here, but modified by distance or something?
    ...
    int16_t yShift = yShiftBits
                         ? int16_t((yShiftBits & 16 ? -(yShiftBits & 15)
                                                    : (yShiftBits & 15)) *
                                   max(scale, 1.0) * 2.0 * invTransformYT)
                         : 0;
    // The above didn't work without float math, didn't feel like figuring out
    // the ridiculous type casting
    ...
    result.drawStartY = ssY < 0 ? 0 : ssY; // Because of these checks, we can
                                           // store them in 1 byte stuctures
    ...
    // These float divisions happen just once per sprite, hopefully that's not
    // too bad. There used to be an option to set the precision of sprites but
    // it didn't seem to make any difference
    float stepXf = (float)result.mminfo.width / spriteWidth;
    float stepYf = result.mminfo.width * invHeight;

    result.texXInit =
        (result.drawStartX - ssX) *
        stepXf; // This unfortunately needs float because of precision glitches
  ...
  template <uint8_t InternalStateBytes>
  void drawSprites(RcPlayer *player, RcSpriteGroup<InternalStateBytes> *group,
                   Arduboy2Base *arduboy) {
    uint8_t usedSprites = group->sortSprites(player->posX, player->posY);

    // Buffers, we pull them out like this just to make it a little easier
    // (might remove later)
    ...
    // after sorting the sprites, do the projection and draw them. We know all
    // sprites in the array are active, since we're looping against the sorted
    // array.
    for (uint8_t i = 0; i < usedSprites; i++) {
      // Get the current sprite so we don't have to dereference multiple
      // pointers
      ...
      if (drawData.stepX == 0 && drawData.stepY == 0)
        continue;
      ...
      // ------- BEGIN CRITICAL SECTION -------------
      // For every strip (x)
      for (uint8_t x = drawData.drawStartX; x < drawData.drawEndX; x++) {
        // If the sprite is hidden, skip this line. Lots of calculations
        // bypassed!
        if (drawData.transformY >= distCache[x >> 1])
          goto SKIPSPRITESTRIPE;

        uint8_t tx = texX.getInteger();

        READTEXSTRIP(spritesheet + fr * 172 + tx * drawData.mminfo.bytes +
                         drawData.mminfo.offset,
                     texData);
        READTEXSTRIP(spritesheet_Mask + fr * 172 + tx * drawData.mminfo.bytes +
                         drawData.mminfo.offset,
                     texMask);
        // texMask = 0xFFFFFFFF;
        texData >>= preshift;
        ...
        RcShadeInfo shading =
            this->calculateShading(drawData.transformY, x, this->spriteShading);
        uint8_t shade = shading.shading;

        // These five variables (including
        // texData+texMask) are needed as part of
        // the loop unrolling system
        uint16_t bofs;
        uint8_t texByte;
        uint8_t maskByte;
...
#define _SPRITEREADSCRBYTE()                                                   \
  bofs = thisWallByte * WIDTH + x;                                             \
  texByte = sbuffer[bofs];                                                     \
  maskByte = 0;
// Write previously read screen byte, go to next byte
#define _SPRITEWRITESCRNEXT()                                                  \
  if (shading.type == RcShadingType::Black) {                                  \
    sbuffer[bofs] = (texByte & (shade | ~maskByte));                           \
  } else {                                                                     \
    sbuffer[bofs] = (texByte | (shade & maskByte));                            \
  }                                                                            \
  thisWallByte++;
...
#define _SPRITEBITUNROLL(bm, nbm)                                              \
  ...
  if (accum < accustep) {                                                      \
    ...
    texMask >>= 1;                                                             \
        ...
        // Last byte, but only need to do it if we end in the middle of a byte
        // and don't simply span one byte
        ...
        // Funny hack; code is written for loop unrolling first, so we have to
        // kind of "fit in" to the macro system
        if ((drawData.drawStartY & 7) == 0)
          thisWallByte--;

        do {
          uint8_t bidx = y & 7;

          // Every new byte, save the current (previous) byte and load the new
          // byte from the screen. This might be wasteful, as only the first
          // and last byte technically need to pull from the screen.
          if (bidx == 0) {
            _SPRITEWRITESCRNEXT();
            _SPRITEREADSCRBYTE();
          }
        ...
        // The above loop specifically CAN'T reach the last byte, so although
        // it's wasteful in the case of a sprite ending at the bottom of the
        // screen, it's still better than always incurring an if statement...
        // maybe. if(drawData.drawEndY & 7)
        _SPRITEWRITESCRNEXT();
